...
export function ArticleQuestionFolderDrawer({ article, onClose, onPractice }: { article: Article; onClose: () => void; onPractice: (questions: PublicQuizQuestion[], title: string) => void }) {
  ...
  const startWrongQuestionPractice = async () => {
    ...
    setError(null);
    try {
      const response = await fetch(`/api/articles/${article.id}/wrong-question-practice`, { method: 'POST' });
      const data = await response.json();
  ...
  const toggleAnalysis = async () => {
    ...
    try {
      const response = await fetch(`/api/articles/${article.id}/question-learning-analysis`);
      const data = await response.json();
    ...
    <div className="fixed inset-0 z-50">
      ...
      <aside aria-label="本篇收藏夹" className="absolute right-0 top-0 flex h-full w-full max-w-xl flex-col border-l-2 border-slate-800 bg-[#fff9e8] shadow-[-5px_0_0_#7d9b68]">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-5">
          ...
          <button type="button" onClick={toggleAnalysis} className="mb-4 flex w-full items-center justify-center gap-2 border-2 border-emerald-800 bg-[#e2f3d0] px-4 py-3 text-sm font-black text-emerald-900 shadow-[2px_2px_0_#166534] transition hover:bg-[#cfeab5]">
          ...
          {isAnalysisOpen && (
            <section className="mb-5 border-2 border-slate-800 bg-[#f3f8e9] p-4">
              ...
              {analysisLoading ? (
                <div className="flex justify-center py-6 text-emerald-800"><Loader2 className="h-5 w-5 animate-spin" /></div>
              ) : analysisError ? (
                <p className="mt-3 border-2 border-[#b94d3c] bg-[#ffe1d6] p-3 text-sm text-[#8b2c21]">{analysisError}</p>
              ) : analysis ? (
                <div className="mt-3 space-y-4">
                  <div className="grid grid-cols-2 gap-2 text-xs md:grid-cols-4">
                    <div className="border border-emerald-800 bg-[#fffdf4] p-2"><b className="block text-lg text-emerald-900">{analysis.overview.bookmarkedCount}</b>收藏题</div>
                    <div className="border border-emerald-800 bg-[#fffdf4] p-2"><b className="block text-lg text-emerald-900">{analysis.overview.attemptedQuestionCount}</b>已练题</div>
                    <div className="border border-emerald-800 bg-[#fffdf4] p-2"><b className="block text-lg text-emerald-900">{analysis.overview.accuracy === null ? '—' : `${analysis.overview.accuracy}%`}</b>总正确率</div>
                    <div className="border border-emerald-800 bg-[#fffdf4] p-2"><b className="block text-lg text-[#9f3426]">{analysis.overview.wrongCount}</b>累计错误</div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-1.5">
                      {analysis.typePerformance.map((item) => (
                        <p key={item.key} className="flex justify-between border border-slate-300 bg-[#fffdf4] px-2 py-1.5 text-xs text-slate-700"><span>{typeLabels[item.key as QuizQuestionType]}</span><span>{item.attemptCount ? `${item.accuracy}%（错 ${item.wrongCount} 次）` : '暂无作答'}</span></p>
                      ))}
                    </div>
                  </div>
                    ...
                    {analysis.weakPoints.length ? (
                      <div className="space-y-1.5">
                        {analysis.weakPoints.map((item) => (
                          <p key={item.key} className="border border-[#b94d3c] bg-[#ffe1d6] px-2 py-1.5 text-xs text-[#8b2c21]"><b>{knowledgePointLabels[item.key] || item.key}</b>：正确率 {item.accuracy}% ，累计错 {item.wrongCount} 次</p>
                        ))}
                      </div>
                    ) : (
                      <p className="border border-slate-300 bg-[#fffdf4] px-2 py-2 text-xs text-slate-600">至少需要 3 次相关作答才会判定薄弱点；当前数据不足或表现稳定。</p>
                    )}
                  </div>
                </div>
              ) : null}
          ...
          {category === 'wrong' && !loading && questions.length > 0 && (
            <button type="button" onClick={startWrongQuestionPractice} disabled={isCreatingPractice} className="mb-4 flex w-full items-center justify-center gap-2 border-2 border-slate-800 bg-amber-300 px-4 py-3 text-sm font-black text-slate-900 shadow-[3px_3px_0_#7c2d12] transition hover:bg-amber-400 disabled:cursor-wait disabled:opacity-60">
              {isCreatingPractice ? <><Loader2 className="h-4 w-4 animate-spin" /> 组卷中</> : '随机练习错题（最多 6 题）'}
            </button>
          )}
            ...
            <div className="space-y-3">
              {questions.map((question) => (
                <article key={question.id} className="border-2 border-slate-800 bg-[#fffdf4] p-4 shadow-[2px_2px_0_#7d9b68]">
                  ...
                  <button type="button" onClick={() => onPractice([{ id: question.id, type: question.type, question: question.question, options: question.options }], '收藏题练习')} className="mt-3 border-2 border-emerald-800 bg-[#e2f3d0] px-3 py-1.5 text-xs font-black text-emerald-900 transition hover:bg-[#cfeab5]">
                    练习此题
                  </button>
                </article>
              ))}
